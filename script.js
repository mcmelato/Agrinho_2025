...
// --- Quiz Interativo ---
    ...
    if (contactForm) {
        ...
        contactForm.addEventListener('submit', async (event) => {
            ...
            try {
                const formData = new FormData(contactForm); // Pega os dados do formulário
            
                const response = await fetch(contactForm.action, { // Envia para a URL do Formspree
                    method: 'POST',
                    body: formData,
                    headers: {
                        'Accept': 'application/json' // Importante para o Formspree responder corretamente
                    }
                });
            
                if (response.ok) { // Se a resposta do Formspree for sucesso (código 2xx)
                    displayModal('Mensagem enviada com sucesso! Agradecemos seu contato.', 'success', 3000);
                    contactForm.reset();
                } else { // Se o Formspree retornar um erro
                    const errorData = await response.json(); // Tenta ler a mensagem de erro
                    console.error('Erro ao enviar formulário para Formspree:', errorData);
                    displayModal(`Ocorreu um erro ao enviar sua mensagem: ${errorData.error || 'Erro desconhecido'}`, 'error');
                }
            } catch (error) {
                console.error('Erro de rede ou ao processar o envio:', error);
                displayModal('Ocorreu um erro de conexão. Por favor, tente novamente mais tarde.', 'error');
            } finally {
